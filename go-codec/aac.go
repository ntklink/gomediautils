...
// Decode parses a 7 byte ADTS header. It reports an error when the buffer is
// shorter than a header.
func (frame *ADTS_Frame_Header) Decode(aac []byte) error {
	if len(aac) < 7 {
		return errors.New("aac: adts header shorter than 7 bytes")
	}
...
func (frame *ADTS_Frame_Header) Encode() []byte {
	...
	hdr[3] = hdr[3] | frame.Variable_Header.Copyright_identification_bit<<3 | frame.Variable_Header.copyright_identification_start<<2 | byte(frame.Variable_Header.Frame_length>>11)&0x03
	hdr[4] = byte(frame.Variable_Header.Frame_length >> 3)
	hdr[5] = byte((frame.Variable_Header.Frame_length&0x07)<<5) | byte(frame.Variable_Header.Adts_buffer_fullness>>6)&0x1F
	hdr[6] = byte(frame.Variable_Header.Adts_buffer_fullness&0x3F)<<2 | frame.Variable_Header.Number_of_raw_data_blocks_in_frame&0x03
	return hdr
}

// SampleToAACSampleIndex maps a sample rate to its ADTS sampling frequency
// index, or -1 when the rate has no index.
...
// AACSampleIdxToSample maps an ADTS sampling frequency index to its sample
// rate, or 0 when the index is reserved.
...
func ConvertADTSToASC(frame []byte) (*AudioSpecificConfiguration, error) {
	...
	if err := adts.Decode(frame); err != nil {
		return nil, err
	}
...
func ConvertASCToADTS(asc []byte, aacbytes int) (*ADTS_Frame_Header, error) {
	...
	aac_adts.Variable_Header.Adts_buffer_fullness = 0x7FF // VBR
