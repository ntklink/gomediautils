...
func (vps *VPS) Decode(nalu []byte) error {
	...
	if vps.Vps_num_layer_sets_minus1 > 1023 {
		bs.Fail(errors.New("h265: vps_num_layer_sets_minus1 > 1023"))
		return bs.Err()
	}
...
// nalu without startcode
func (sps *H265RawSPS) Decode(nalu []byte) error {
	sodb := CovertRbspToSodb(nalu)
	bs := NewBitStream(sodb)
	hdr := H265NaluHdr{}
	hdr.Decode(bs)
	...
	if num_short_term_ref_pic_sets > 64 {
		bs.Fail(errors.New("h265: num_short_term_ref_pic_sets beyond HEVC_MAX_SHORT_TERM_REF_PIC_SETS"))
		return bs.Err()
	}
	...
	if bs.GetBit() == 1 {
		...
		if num_long_term_ref_pics_sps > 32 {
			bs.Fail(errors.New("h265: num_long_term_ref_pics_sps > 32"))
			return bs.Err()
		}
...
func skip_hrd_parameters(cprms_present_flag uint8, max_sub_layers_minus1 uint32, bs *BitStream) {
	...
	for i := 0; i <= int(max_sub_layers_minus1); i++ {
		...
		if fixed_pic_rate_general_flag == 0 {
			...
			// inferred to be equal to 1 when fixed_pic_rate_general_flag == 1
		...
		if low_delay_hrd_flag == 0 {
			...
			if cpb_cnt_minus1 > 31 {
				bs.Fail(errors.New("h265: cpb_cnt_minus1 > 31"))
				return
			}
		}
...
func parse_rps(rps_idx int, nums_rps uint64, num_delta_pocs *[64]uint32, bs *BitStream) {
	if rps_idx > 0 && bs.GetBit() > 0 {
		if rps_idx > int(nums_rps) || rps_idx >= len(num_delta_pocs) {
			bs.Fail(errors.New("h265: short term ref pic set index out of range"))
			return
		}
		bs.SkipBits(1)
		bs.ReadUE()
		...
		if (num_negative_pics+num_positive_pics)*2 > uint64(bs.RemainBits()) {
			bs.Fail(errors.New("h265: short term ref pic set exceeds the available data"))
			return
		}
		if rps_idx >= 0 && rps_idx < len(num_delta_pocs) {
			num_delta_pocs[rps_idx] = uint32(num_negative_pics + num_positive_pics)
		}
...
// nalu without startcode
func (pps *H265RawPPS) Decode(nalu []byte) error {
	sodb := CovertRbspToSodb(nalu)
	bs := NewBitStream(sodb)
	hdr := H265NaluHdr{}
	hdr.Decode(bs)
...
// GetH265Resolution returns the coded resolution of an SPS. It reports an
// error when the SPS is truncated or malformed.
func GetH265Resolution(sps []byte) (width uint32, height uint32, err error) {
	off := naluOffset(sps)
	if off < 0 {
		return 0, 0, errors.New("h265: empty sps nalu")
	}
	h265sps := H265RawSPS{}
	if err = h265sps.Decode(sps[off:]); err != nil {
		return 0, 0, err
	}
	return uint32(h265sps.Pic_width_in_luma_samples), uint32(h265sps.Pic_height_in_luma_samples), nil
}
...
func GetVPSIdWithStartCode(vps []byte) uint8 {
	...
	return GetVPSId(vps[off:])
}

func GetVPSId(vps []byte) uint8 {
	var rawvps VPS
	if err := rawvps.Decode(vps); err != nil {
		return 0
	}
...
func GetH265PPSIdWithStartCode(pps []byte) uint64 {
	...
	return GetH265PPSId(pps[off:])
}

func GetH265PPSId(pps []byte) uint64 {
	var rawpps H265RawPPS
	if err := rawpps.Decode(pps); err != nil {
		return 0
	}
...
// Decode parses an HEVCDecoderConfigurationRecord. It reports an error for a
// truncated or malformed record.
func (hvcc *HEVCRecordConfiguration) Decode(hevc []byte) error {
	...
	// build into a local slice so that a truncated record never leaves the
	// configuration holding half initialised (nil) entries
	arrays := make([]*HVCCNALUnitArray, 0, hvcc.NumOfArrays)
	for i := 0; i < int(hvcc.NumOfArrays); i++ {
		array := new(HVCCNALUnitArray)
		array.Array_completeness = bs.GetBit()
		bs.SkipBits(1)
		array.NAL_unit_type = bs.Uint8(6)
		array.NumNalus = bs.Uint16(16)
		if bs.Err() != nil {
			return bs.Err()
		}
		// every nalu needs at least a 2 byte length prefix, so a count that
		// cannot fit in what is left is malformed; without this a 30 byte
		// record could ask for millions of allocations
		if int(array.NumNalus)*2 > bs.RemainBytes() {
			return errors.New("h265: hvcC nalu count beyond the record")
		}
		array.NalUnits = make([]*NalUnit, 0, array.NumNalus)
		for j := 0; j < int(array.NumNalus); j++ {
			nalu := new(NalUnit)
			nalu.NalUnitLength = bs.Uint16(16)
			nalu.Nalu = bs.GetBytes(int(nalu.NalUnitLength))
			if bs.Err() != nil {
				return bs.Err()
			}
			array.NalUnits = append(array.NalUnits, nalu)
		}
...
func (hvcc *HEVCRecordConfiguration) UpdateSPS(sps []byte) error {
	off := naluOffset(sps)
	if off < 0 {
		return errors.New("h265: empty sps nalu")
	}
	sps = sps[off:]
	var rawsps H265RawSPS
	if err := rawsps.Decode(sps); err != nil {
...
func (hvcc *HEVCRecordConfiguration) UpdatePPS(pps []byte) error {
	off := naluOffset(pps)
	if off < 0 {
		return errors.New("h265: empty pps nalu")
	}
	pps = pps[off:]
	var rawpps H265RawPPS
	if err := rawpps.Decode(pps); err != nil {
...
func (hvcc *HEVCRecordConfiguration) UpdateVPS(vps []byte) error {
	off := naluOffset(vps)
	if off < 0 {
		return errors.New("h265: empty vps nalu")
	}
	vps = vps[off:]
	var rawvps VPS
	if err := rawvps.Decode(vps); err != nil {
