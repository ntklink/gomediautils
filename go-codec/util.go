...
func SplitFrame(frames []byte, onFrame func(nalu []byte) bool) {
	...
	for beg >= 0 {
		...
		if onFrame != nil && (beg+int(sc)) < end && onFrame(frames[beg+int(sc):end]) == false {
			break
		}
		beg = end
		sc = sc2
...
// SplitAACFrame splits an ADTS stream into frames, calling onFrame for each
// complete frame. It stops and reports an error on truncated or invalid input
// (a frame_length below the header size or reaching past the buffer); frames
// found before that are still delivered.
func SplitAACFrame(frames []byte, onFrame func(aac []byte)) error {
	var adts ADTS_Frame_Header
	start := FindSyncword(frames, 0)
	for start >= 0 {
		if start+7 > len(frames) {
			return errors.New("adts header truncated")
		}
		if err := adts.Decode(frames[start:]); err != nil {
			return err
		}
		frameLen := int(adts.Variable_Header.Frame_length)
		if frameLen < 7 {
			return errors.New("invalid adts frame_length")
		}
		if start+frameLen > len(frames) {
			return errors.New("adts frame truncated")
		}
		if onFrame != nil {
			onFrame(frames[start : start+frameLen])
		}
...
// or may not be prefixed with a start code; when there is none the nalu is
// assumed to start at offset 0. It returns -1 when buf holds no nalu header.
func naluOffset(buf []byte) int {
	loc, sc := FindStartCode(buf, 0)
	if loc < 0 {
		loc, sc = 0, 0
	}
	off := loc + int(sc)
...
// H264NaluType returns the nalu type of the first nalu in h264, which may be
// given with or without a start code. H264_NAL_RESERVED is returned when the
// buffer holds no nalu header.
func H264NaluType(h264 []byte) H264_NAL_TYPE {
	off := naluOffset(h264)
	if off < 0 {
		return H264_NAL_RESERVED
	}
	return H264_NAL_TYPE(h264[off] & 0x1F)
}
...
// H265NaluType returns the nalu type of the first nalu in h265, which may be
// given with or without a start code.
func H265NaluType(h265 []byte) H265_NAL_TYPE {
	off := naluOffset(h265)
	if off < 0 {
		return H265_NAL_TYPE(0)
	}
	return H265_NAL_TYPE((h265[off] >> 1) & 0x3F)
}
...
func GetH264FirstMbInSlice(nalu []byte) uint64 {
	off := naluOffset(nalu)
	if off < 0 || off+1 >= len(nalu) {
		return 0
	}
	bs := NewBitStream(nalu[off+1:])
...
func GetH265FirstMbInSlice(nalu []byte) uint64 {
	off := naluOffset(nalu)
	if off < 0 || off+2 >= len(nalu) {
		return 0
	}
	bs := NewBitStream(nalu[off+2:])
	sliceHdr := &SliceHeader{}
	sliceHdr.Decode(bs)
	return sliceHdr.First_mb_in_slice
}
...
// CovertRbspToSodb removes emulation prevention bytes (00 00 03 -> 00 00).
// It always returns a newly allocated slice.
func CovertRbspToSodb(rbsp []byte) []byte {
	sodb := make([]byte, 0, len(rbsp))
	zeros := 0
