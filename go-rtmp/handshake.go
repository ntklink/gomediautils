...
// errNoOutput is reported when a handshake has to write but no output callback
// was installed with SetOutput
var errNoOutput = errors.New("rtmp: output is not set")
...
func (chs *clientHandShake) start() error {
	var c0c1 []byte
	chs.state = CLIENT_S0
	if chs.simpleHs {
		c0c1 = makeC0()
	...
	return chs.output(c0c1)
}

// input feeds handshake bytes, it returns the number of bytes consumed and the error of the C2
// write, if any. Bytes left over after the handshake completes (typically the first rtmp chunk)
// must be fed to the chunk reader by the caller, also when an error is returned
func (chs *clientHandShake) input(data []byte) (readBytes int, err error) {
	for len(data) > 0 {
		switch chs.state {
		case CLIENT_S0:
			chs.version = data[0]
		...
		case CLIENT_S1:
			...
			if chs.output == nil {
				return readBytes, errNoOutput
			}
			if err := chs.output(c2); err != nil {
		...
		case CLIENT_S2:
			...
			return readBytes, nil
		default:
			// handshake already done, nothing consumed
			return readBytes, nil
		}
	}
...
// input feeds handshake bytes, it returns the number of bytes consumed and the error of the
// S0S1S2 write, if any
func (shs *serverHandShake) input(data []byte) (readBytes int, err error) {
	for len(data) > 0 {
		switch shs.state {
		case SERVER_C0:
			shs.version = data[0]
		...
		case SERVER_C1:
			...
			if err := shs.output(s0s1s2); err != nil {
				return readBytes, err
			}
