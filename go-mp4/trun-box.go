...
// maxTrunSampleCount bounds the sample_count of a trun box that carries no
// per-sample fields (and therefore cannot be validated against its size).
const maxTrunSampleCount = 1 << 24
...
func (trun *TrackRunBox) Decode(r io.Reader, size uint32, dataOffset uint32) (offset int, err error) {
	...
	entrySize := 0
	for _, f := range []uint32{TR_FLAG_DATA_SAMPLE_DURATION, TR_FLAG_DATA_SAMPLE_SIZE, TR_FLAG_DATA_SAMPLE_FLAGS, TR_FLAG_DATA_SAMPLE_COMPOSITION_TIME} {
		if trunFlags&f > 0 {
			entrySize += 4
		}
	}
	if entrySize > 0 {
		if err = checkTableSize(trun.SampleCount, entrySize, int64(len(buf)-n)); err != nil {
			return 0, err
		}
	} else if trun.SampleCount > maxTrunSampleCount {
		return 0, errors.New("mp4: trun sample_count too large")
	}
...
func decodeTrunBox(demuxer *MovDemuxer, size uint32) (err error) {
	...
	for _, entry := range trun.EntryList.entrys {
		...
		if trun.Box.Version == 1 {
			cts = int64(int32(entry.sampleCompositionTimeOffset))
		} else {
	...
	// several trun boxes may follow each other inside one traf: the next run
	// continues from where this one stopped.
	demuxer.currentTrack.startDts = nextDts
