...
func (sidx *SegmentIndexBox) Decode(r io.Reader) (offset int, err error) {
	...
	buf, err := readBoxPayload(r, sidx.Box.Box.Size, FullBoxLen)
	if err != nil {
		return
	}
	n := 0
	fixed := 8 + 4 // ids + reserved/reference_count
	if sidx.Box.Version == 0 {
		fixed += 8
	} else {
		fixed += 16
	}
	if err = checkRemain(buf, n, fixed); err != nil {
		return
	}
	...
	n += 2
	if err = checkRemain(buf, n, int(sidx.ReferenceCount)*12); err != nil {
		return
	}
