...
func (hdlr *HandlerBox) Decode(r io.Reader, size uint64) (offset int, err error) {
	...
	buf, err := readBoxPayload(r, size, FullBoxLen)
	if err != nil {
		return 0, err
	}
	// pre_defined(4) handler_type(4) reserved(12)
	if err = checkRemain(buf, 0, 20); err != nil {
		return 0, err
	}
	offset = 4
...
func getHandlerType(cid MP4_CODEC_TYPE) (HandlerType, error) {
	switch cid {
	...
	case MP4_CODEC_AAC, MP4_CODEC_G711A, MP4_CODEC_G711U,
		...
		return soun, nil
	default:
		return HandlerType{}, unsupportedCodec(cid)
	}
}
