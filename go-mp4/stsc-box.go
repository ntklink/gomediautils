...
func (stsc *SampleToChunkBox) Decode(r io.Reader) (offset int, err error) {
	...
	remain, err := stsc.box.tableRemain(4)
	if err != nil {
		return
	}
	if err = checkTableSize(stsc.stscentrys.entryCount, 12, remain); err != nil {
		return
	}
	stsc.stscentrys.entrys = make([]stscEntry, stsc.stscentrys.entryCount)
	buf := make([]byte, int(stsc.stscentrys.entryCount)*12)
...
func decodeStscBox(demuxer *MovDemuxer, size uint64) (err error) {
	track, err := demuxer.lastStbl()
	if err != nil {
		return err
	}
	stsc := SampleToChunkBox{box: &FullBox{Box: &BasicBox{Size: size}}}
