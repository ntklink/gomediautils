...
func decodeAudioSampleEntry(demuxer *MovDemuxer) (err error) {
	...
	if _, err = entry.Decode(demuxer.reader); err != nil {
		return
	}
...
func makeStsd(track *mp4track, handler_type HandlerType) ([]byte, error) {
	...
	if len(track.extraData) == 0 {
		if track.cid == MP4_CODEC_AAC || track.cid == MP4_CODEC_H264 || track.cid == MP4_CODEC_H265 {
			if track.extra == nil {
				return nil, fmt.Errorf("mp4: track %d has no codec configuration", track.trackId)
			}
			if extraData, err = track.extra.export(); err != nil {
				return nil, fmt.Errorf("mp4: track %d: %w", track.trackId, err)
			}
			if len(extraData) == 0 {
				return nil, fmt.Errorf("mp4: track %d has an empty codec configuration", track.trackId)
			}
		}
	} else {
	...
	case MP4_CODEC_AAC, MP4_CODEC_MP2, MP4_CODEC_MP3:
		if avbox, err = makeEsdsBox(track.trackId, track.cid, extraData); err != nil {
			return nil, err
		}
	case MP4_CODEC_OPUS:
	...
	codecName, err := getCodecNameWithCodecId(track.cid)
	if err != nil {
		return nil, err
	}
	...
	var offset int
	switch {
	case handler_type.equal(vide):
		entry := NewVisualSampleEntry(codecName)
	...
	case handler_type.equal(soun):
		entry := NewAudioSampleEntry(codecName)
		...
		offset, se = entry.Encode()
	default:
		return nil, fmt.Errorf("mp4: track %d has unsupported handler type %s", track.trackId, string(handler_type[:]))
	}
...
func makeEsdsBox(tid uint32, cid MP4_CODEC_TYPE, extraData []byte) ([]byte, error) {
	esd, err := makeESDescriptor(uint16(tid), cid, extraData)
	if err != nil {
		return nil, err
	}
...
func decodeEsdsBox(demuxer *MovDemuxer, size uint32) (err error) {
	...
	buf, err := readBoxPayload(demuxer.reader, uint64(size), FullBoxLen)
	if err != nil {
		return
	}
	vosdata, err := decodeESDescriptor(buf, track)
	if err != nil {
		return err
	}
	if track.extra != nil {
		return track.extra.load(vosdata)
	}
	return nil
}
