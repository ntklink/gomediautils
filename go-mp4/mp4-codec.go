...
// ErrUnsupportedCodec is reported for a codec id that cannot be carried in an
// mp4 track, or that the box being written has no representation for. Wrap
// aware callers can test for it with errors.Is.
var ErrUnsupportedCodec = errors.New("mp4: unsupported codec id")

func unsupportedCodec(cid MP4_CODEC_TYPE) error {
	return fmt.Errorf("%w: %d", ErrUnsupportedCodec, int(cid))
}
...
func getCodecNameWithCodecId(cid MP4_CODEC_TYPE) ([4]byte, error) {
	switch cid {
	case MP4_CODEC_H264:
		return [4]byte{'a', 'v', 'c', '1'}, nil
	case MP4_CODEC_H265:
		return [4]byte{'h', 'v', 'c', '1'}, nil
	case MP4_CODEC_AAC, MP4_CODEC_MP2, MP4_CODEC_MP3:
		return [4]byte{'m', 'p', '4', 'a'}, nil
	case MP4_CODEC_G711A:
		return [4]byte{'a', 'l', 'a', 'w'}, nil
...
func getBojecttypeWithCodecId(cid MP4_CODEC_TYPE) (uint8, error) {
	switch cid {
	case MP4_CODEC_H264:
		return 0x21, nil
	case MP4_CODEC_H265:
...
func getCodecIdByObjectType(objType uint8) MP4_CODEC_TYPE {
	...
	default:
		// unknown object type: the caller must treat 0 as "unsupported"
		return 0
	}
}
