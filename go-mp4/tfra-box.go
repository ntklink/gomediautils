...
func (tfra *TrackFragmentRandomAccessBox) Decode(r io.Reader) (offset int, err error) {
	...
	buf, err := readBoxPayload(r, tfra.Box.Box.Size, FullBoxLen)
	if err != nil {
		return 0, err
	}
	n := 0
	if err = checkRemain(buf, n, 12); err != nil {
		return 0, err
	}
	...
	n += 4
	entrySize := 8 + int(tfra.LengthSizeOfTrafNum+tfra.LengthSizeOfTrunNum+tfra.LengthSizeOfSampleNum+3)
	if tfra.Box.Version == 1 {
		entrySize += 8
	}
	if err = checkTableSize(tfra.NumberOfEntry, entrySize, int64(len(buf)-n)); err != nil {
		return 0, err
	}
