...
func (ctts *CompositionOffsetBox) Decode(r io.Reader) (offset int, err error) {
	...
	ctts.ctts.version = ctts.box.Version
	ctts.ctts.entryCount = binary.BigEndian.Uint32(entryCountBuf)
	remain, err := ctts.box.tableRemain(4)
	if err != nil {
		return
	}
	if err = checkTableSize(ctts.ctts.entryCount, 8, remain); err != nil {
		return
	}
	ctts.ctts.entrys = make([]cttsEntry, ctts.ctts.entryCount)

	buf := make([]byte, int(ctts.ctts.entryCount)*8)
...
func decodeCttsBox(demuxer *MovDemuxer, size uint64) (err error) {
	track, err := demuxer.lastStbl()
	if err != nil {
		return err
	}
	ctts := CompositionOffsetBox{box: &FullBox{Box: &BasicBox{Size: size}}}
