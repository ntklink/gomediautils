...
func (head *RtpHdr) Encode() []byte {
	data := make([]byte, RTP_FIX_HEAD_LEN+4*len(head.CSRC))
	head.encodeTo(data)
	return data
}

// encodeTo writes the header into data, which must be at least
// RTP_FIX_HEAD_LEN+4*len(CSRC) bytes long.
func (head *RtpHdr) encodeTo(data []byte) {
	cc := uint8(len(head.CSRC))
	data[0] = byte(0x80) | (head.PaddingFlag & 0x01 << 5) | (head.ExtensionFlag & 0x01 << 4) | (cc & 0x0F)
	...
	for i, csrc := range head.CSRC {
		binary.BigEndian.PutUint32(data[RTP_FIX_HEAD_LEN+4*i:], csrc)
	}
}
