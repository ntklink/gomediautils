...
func (pkt *SourceDescription) Decode(data []byte) error {
	...
	offset := 4
	pkt.Chunks = pkt.Chunks[:0]
	for i := 0; i < int(pkt.SC); i++ {
		if offset+6 > end {
			return errors.New("sdes rtcp packet need more data")
		}
		chk := SDESChunk{}
		chk.SSRC = binary.BigEndian.Uint32(data[offset:])
		offset += 4
		chk.Item = &ChunkItem{
			Type:   data[offset],
			Length: data[offset+1],
		}
		if offset+2+int(chk.Item.Length) > end {
			return errors.New("sdes rtcp item exceeds packet")
		}
		chk.Item.Txt = make([]byte, chk.Item.Length)
		copy(chk.Item.Txt, data[offset+2:offset+2+int(chk.Item.Length)])
		offset += 2 + int(chk.Item.Length)
		// skip the terminating null item and padding to the next 32-bit boundary
		for offset < end && data[offset] == 0 {
			offset++
		}
		pkt.Chunks = append(pkt.Chunks, chk)
	}
	return nil
}
