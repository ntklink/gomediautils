...
func (ctx *RtcpContext) getReportBlock() ReportBlock {
	...
	// the cumulative loss is a signed 24 bit field; more received than
	// expected (duplicates) would make it negative, saturate instead
	lost := int64(expected) - int64(ctx.received)
	if lost < 0 {
		lost = 0
	} else if lost > 0x7FFFFF {
		lost = 0x7FFFFF
	}
	// compute the intervals signed: expectPrior/receivedPrior can exceed the
	// current counters after a reset and would wrap around in uint32
	expectedInterval := int64(expected) - int64(ctx.expectPrior)
	ctx.expectPrior = expected
	receivedInterval := int64(ctx.received) - int64(ctx.receivedPrior)
