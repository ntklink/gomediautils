...
// errNotImplemented marks the entry points of this package that are still
// stubs, so that a caller is not told the operation succeeded.
var errNotImplemented error = errors.New("rtsp: not implemented")
...
func (req *RtspRequest) parse(data string) (int, error) {
	return req.parseBytes([]byte(data))
}

// parseBytes parses a request from raw bytes; only the header portion is
// converted to a string, the body is sliced by Content-Length.
func (req *RtspRequest) parseBytes(raw []byte) (int, error) {
	loc := bytes.Index(raw, []byte("\r\n\r\n"))
	if loc == -1 {
		return 0, errNeedMore
	}
	body := raw[loc+4:]
	strs := strings.Split(string(raw[:loc]), "\r\n")
	if len(strs) <= 1 {
		return 0, errors.New("illegal rtsp request")
	}

	if err := req.parseFirstLine(strs[0]); err != nil {
		return 0, err
	}
	if req.Fileds == nil {
		req.Fileds = make(HeadFiled)
	}
	for i := 1; i < len(strs); i++ {
		kv := strings.SplitN(strs[i], ":", 2)
		if len(kv) < 2 {
			// header line without ':' , ignore it
			continue
		}
	...
	if content_length, found := req.Fileds["Content-Length"]; found {
		...
		if err != nil || length < 0 {
			return 0, errors.New("illegal Content-Length")
		}
		if length > len(body) {
			return 0, errNeedMore
		}
		req.Body = string(body[:length])
	}
...
func (res *RtspResponse) parse(data string) (int, error) {
	return res.parseBytes([]byte(data))
}

func (res *RtspResponse) parseBytes(raw []byte) (int, error) {
	loc := bytes.Index(raw, []byte("\r\n\r\n"))
	if loc == -1 {
		return 0, errNeedMore
	}
...
func (res *RtspResponse) parseFirstLine(firstLine string) error {
	...
	statusCode, err := strconv.Atoi(strings.TrimSpace(sets[1]))
	if err != nil || statusCode < 100 || statusCode > 999 {
		return errors.New("rtsp parse response failed,illegal status code: " + sets[1])
	}
