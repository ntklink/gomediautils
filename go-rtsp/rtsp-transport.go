...
func (transport *RtspTransport) DecodeString(data string) error {
	...
	for _, item := range items {
		...
		case "mode":
			if value == "" {
				return errors.New("transport: mode without value")
			}
			transport.mode = strings.Trim(value, "\"")
		case "client_port":
			if _, err := fmt.Sscanf(value, "%d-%d", &transport.Client_ports[0], &transport.Client_ports[1]); err != nil {
				if _, err2 := fmt.Sscanf(value, "%d", &transport.Client_ports[0]); err2 != nil {
					return fmt.Errorf("transport: illegal client_port %q", value)
				}
				transport.Client_ports[1] = transport.Client_ports[0] + 1
			}
		case "server_port":
			if _, err := fmt.Sscanf(value, "%d-%d", &transport.Server_ports[0], &transport.Server_ports[1]); err != nil {
				if _, err2 := fmt.Sscanf(value, "%d", &transport.Server_ports[0]); err2 != nil {
					return fmt.Errorf("transport: illegal server_port %q", value)
				}
				transport.Server_ports[1] = transport.Server_ports[0] + 1
			}
		case "interleaved":
			if _, err := fmt.Sscanf(value, "%d-%d", &transport.Interleaved[0], &transport.Interleaved[1]); err != nil {
				if _, err2 := fmt.Sscanf(value, "%d", &transport.Interleaved[0]); err2 != nil {
					return fmt.Errorf("transport: illegal interleaved %q", value)
				}
				transport.Interleaved[1] = transport.Interleaved[0] + 1
			}
		}
	}
	return nil
