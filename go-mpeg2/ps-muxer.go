...
type PSMuxer struct {
	...
	// OnPacket receives every program stream pack. The slice is owned by the
	// muxer and is overwritten by the next pack: copy it if it has to outlive
	// the callback.
...
func (muxer *PSMuxer) Write(sid uint8, frame []byte, pts uint64, dts uint64) error {
	...
	pts = pts * 90
	if muxer.bsw == nil {
		muxer.bsw = codec.NewBitStreamWriter(1024)
	}
	bsw := muxer.bsw
	bsw.Reset()
	...
	if dts > 3600 {
		...
		pack.System_clock_reference_base = 0
	}
