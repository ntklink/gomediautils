...
// pesOptionalFieldsSize returns the number of bytes the optional fields
// selected by the PES header flags occupy inside PES_header_data_length.
func (pkg *PesPacket) pesOptionalFieldsSize() int {
	need := 0
	if pkg.PTS_DTS_flags&0x02 == 0x02 {
		need += 5
	}
...
func (pkg *PesPacket) Decode(bs *codec.BitStream) error {
	...
	hdrLen := int(pkg.PES_header_data_length)
	// PES_packet_length counts everything after itself: the 3 fixed header
	// bytes plus PES_header_data_length bytes plus the payload.
	if pkg.PES_packet_length != 0 && int(pkg.PES_packet_length) < 3+hdrLen {
		return errParser
	}
	// the optional fields selected by the flags must fit in the header
	if pkg.pesOptionalFieldsSize() > hdrLen {
		return errParser
	}
	if bs.RemainBytes() < hdrLen {
		bs.UnRead(9 * 8)
		return errNeedMore
	}
	bs.Markdot()
	...
	// skip remaining header bytes (extension / stuffing); never negative
	if skip := hdrLen*8 - bs.DistanceFromMarkDot(); skip > 0 {
		bs.SkipBits(skip)
	}
	if bs.Err() != nil {
		// unreachable: the optional fields were checked to fit in
		// PES_header_data_length and the header was checked to be present
		return errParser
	}

	if pkg.PES_packet_length == 0 {
		// unbounded PES (only legal for video in TS): the payload is
		// delimited by the transport (PUSI) or by the next start code.
	...
	dataLen := int(pkg.PES_packet_length) - 3 - hdrLen

	if bs.RemainBytes() < dataLen {
		pkg.Pes_payload = bs.RemainData()
		bs.UnRead((9 + hdrLen) * 8)
		return errNeedMore
	}
...
func (pkg *PesPacket) DecodeMpeg1(bs *codec.BitStream) error {
	...
	case bs.NextBits(4) == 0x03:
		...
		pkg.Dts = pkg.Dts<<15 | bs.GetBits(15)
		bs.SkipBits(1)
	case bs.NextBits(8) == 0x0F:
		bs.SkipBits(8)
	default:
		if bs.Err() != nil {
			// ran out of data inside the header; only possible for an
			// unbounded packet, more data may complete it
			return errNeedMore
		}
		return errParser
	}
	if bs.Err() != nil {
		// the header announced fields the packet does not contain
		return errNeedMore
	}
	loc := bs.DistanceFromMarkDot() / 8
	if pkg.PES_packet_length != 0 && int(pkg.PES_packet_length) < loc {
		return errParser
	}
	if pkg.PES_packet_length == 0 ||
