...
// errAdaptationLength reports an adaptation_field_length that does not agree
// with the amount of data in the packet.
var errAdaptationLength = errors.New("mpeg2: adaptation_field_length larger than the ts packet")
...
func (adaptation *Adaptation_field) Decode(bs *codec.BitStream) error {
	...
	// decode from a sub-slice of exactly Adaptation_field_length bytes so that
	// no field read can go past the adaptation field
	field := bs.RemainData()
	if len(field) < int(adaptation.Adaptation_field_length) {
		return errAdaptationLength
	}
	field = field[:int(adaptation.Adaptation_field_length)]
	bs.SkipBits(len(field) * 8)
	return adaptation.decodeField(codec.NewBitStream(field))
}

// decodeField decodes the adaptation field from a bit stream that holds
// exactly Adaptation_field_length bytes. Every read is therefore bounded by
// the field itself and a field that is too short for the flags it announces
// shows up as a single error at the end.
func (adaptation *Adaptation_field) decodeField(bs *codec.BitStream) error {
	...
	if adaptation.Adaptation_field_extension_flag == 1 {
		...
		bs.Markdot()
		adaptation.decodeExtension(bs)
		// skip whatever part of the extension the flags did not describe
		if rest := int(adaptation.Adaptation_field_extension_length)*8 - bs.DistanceFromMarkDot(); rest > 0 {
			bs.SkipBits(rest)
		}
	}
	// whatever is left is stuffing
	return bs.Err()
}

func (adaptation *Adaptation_field) decodeExtension(bs *codec.BitStream) {
...
func ReadSection(sectionType PAT_TID, bs *codec.BitStream) (interface{}, error) {
	...
	for !bs.EOS() && bs.NextBits(8) == 0xff {
		bs.SkipBits(8)
	}
	if bs.EOS() {
		return nil, errors.New("mpeg2: ts payload carries no section")
	}
	...
	return nil, fmt.Errorf("mpeg2: unsupported section table id %d", sectionType)
}
...
func (pat *Pat) Decode(bs *codec.BitStream) error {
	...
	// Section_length counts everything after itself:
	// 5 fixed bytes + N*4 program entries + 4 bytes CRC
	if pat.Section_length < 9 {
		return errors.New("pat Section_length too small")
	}
	if int(pat.Section_length) > bs.RemainBytes() {
		return errors.New("pat Section_length exceeds available data")
	}
...
func (pmt *Pmt) Decode(bs *codec.BitStream) error {
	...
	if pmt.Table_id != uint8(TS_TID_PMS) {
		return errors.New("table id is Not TS_TID_PMS")
	}
	// Section_length counts everything after itself:
	// 9 fixed bytes + program info + stream loop + 4 bytes CRC
	if pmt.Section_length < 13 {
		return errors.New("pmt Section_length too small")
	}
	if int(pmt.Section_length) > bs.RemainBytes() {
		return errors.New("pmt Section_length exceeds available data")
	}
	...
	loopLen := int(pmt.Section_length) - 9 - 4
	if int(pmt.Program_info_length) > loopLen {
		return errors.New("pmt Program_info_length exceeds Section_length")
	}
	//TODO N loop descriptors
	bs.SkipBits(int(pmt.Program_info_length) * 8)
	loopLen -= int(pmt.Program_info_length)
	for i := 0; i+5 <= loopLen; {
		tmp := StreamPair{}
		...
		if i+5+int(tmp.ES_Info_Length) > loopLen {
			return errors.New("pmt ES_Info_Length exceeds Section_length")
		}
		//TODO N loop descriptors
